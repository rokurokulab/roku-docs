...
水位常量从源码直接读出（`compact.rs`，`runtime_config.rs`）：

| 层 | 触发水位 | 默认值（200K context） |
|----|----------|----------------------|
| L1/L2 机械 collapse / memory splice | 60%（`MID_WATER_TRIGGER_RATIO`） | 120 000 tokens |
| L3a/L3b LLM 级 compact | 75%（`compact_threshold_ratio`） | 150 000 tokens |
| Hard max | — | 2 000 000 tokens |

> [推测] 60/75 两个具体数值来自设计材料（内部笔记，未公开），源码内没有显式的设计依据注释。

---

## 各层的逻辑

**Layer 0 micro-compact** 每轮无条件执行，替换旧 ToolResult 的完整内容为摘要。它不依赖水位，只要有 tool 调用就跑。目的是把 tool output 这类体积大但历史价值低的内容提前清掉，推迟高水位触发的时机。

**Layer 1 机械 collapse** 在 60% 水位触发，直接截断最旧半段对话，插入一行机械摘要替代。没有 LLM 调用，延迟可预期。代价是旧的对话细节永久丢失，摘要质量低。在大多数短 run 场景里这已经够了；60% 触发留出 40% 的缓冲，足以容纳一轮完整 tool loop 的新输入。

**Layer 2 session memory splice** 是比 L1 更好的方案：用从 `LongTermMemoryBackend` 召回的真实 session summary 替换旧消息，保留语义而不是机械截断。接口已实现，`mid_compact_messages` 签名接受 `session_summary: Option<&str>`。但当前 `runtime.rs` 的调用点传入 `None`，L2 实际上不激活，退化到 L1 行为。原因是注入链路（从 memory 子系统打通到 `execute_tool_loop` 的 mid-tier 调用点）尚未完成。

**Layer 3a 远端 compact** 是 `OpenAiResponsesProvider` 专有路径，直接 POST 到 `/responses/compact` unary 端点，90s timeout，不走 SSE。这个端点存在的价值正是规避 SSE hang：服务端返回结构化摘要，整个交互是一次 HTTP 请求/响应，没有流式连接。

**Layer 3b LLM summarizer** 是没有远端 compact 端点时的本地 LLM 摘要路径，300s timeout。它能用，但在 Responses 后端上不应该走，因为 SSE hang 风险就在这里。

**Layer 4 reactive compact** 是最后防线，处理 `ContextWindowExceeded` 错误，每 turn 最多触发一次，压缩后仍超限则直接 fail。
...
当前已实现：L0 micro-compact、L1 机械 collapse、L2 接口、L3a 远端 compact（`OpenAiResponsesProvider::compact_history`）、L3b LLM summarizer、L4 reactive compact、`EstimatorCalibration`（在线校准，最近 8 样本线性校准）、断路器（`MAX_CONSECUTIVE_AUTOCOMPACT_FAILURES = 3`）、`CacheBreakDetector`。

尚未激活：L2 真实 session summary 注入（caller 传 `None`，L2 路径不激活）。

尚未实现：per-tool output cap（Bash 30K / Grep 20K 等）、token 使用率 / compact 触发链的完整 metrics 上报。
