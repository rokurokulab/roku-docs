...
LLM 可以通过调用 `"Agent"` 工具名派发一个 sub-agent——这条路径走独立的消息历史、独立的 step 预算，最终只把一段压缩结果交还给主 loop。

主循环在工具执行阶段识别到 `Agent` 工具时，分派到 `execute_sub_agent`，最大深度硬约束为 1（子 loop 里再派 sub-agent 会被立刻拒绝）。完整的继承规则、预算扣减、结果截断、已知局限见 [Sub-agent](./sub-agent.md)。
